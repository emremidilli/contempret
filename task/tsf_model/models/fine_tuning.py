...
class SequencePredictor(tf.keras.Model):
    ...
    def __init__(
        ...
        self.tre_embedding.trainable = False
        self.sea_embedding.trainable = False
        self.res_embedding.trainable = False

        if self.trend_prompt is not None:
            self.trend_prompt.trainable = True

        if self.seasonality_prompt is not None:
            self.seasonality_prompt.trainable = True

        if self.residual_prompt is not None:
            self.residual_prompt.trainable = True
        ...
        if self.encoder_representation.time2vec is not None:
            self.encoder_representation.time2vec.trainable = tune_time2vec
